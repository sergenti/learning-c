...
*b is a pointer and points to a.
*b stores the location (address in memory) of a

if we change a, *b willalays contain the sam value

DEREFERENCING A POINTER
obtain the value that is in the location of said pointer

& address of
* means 2 things: creating a pointer and dereferencing it
...
int main()
{
    ...
    b = &a; // &a creates a pointer to a. Assigning that to B. aka the "address-of-operator"

    printf("a   = %d\n", a);
    printf("*b  = %d\n", *b);
    // dereferencing the pointer aka the "indirection operator"
    ...
    *b = 300;
    ...
    // when you pass an array to a function it decays as a pointer
...
// power of pointers => it allows us to change value of variable inside functions